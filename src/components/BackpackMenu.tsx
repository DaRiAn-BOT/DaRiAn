...
export default function BackpackMenu({ items, potions, health, maxHealth, equipped, onEquip, onUsePotion, onClose }: Props) {
  ...
  return <div className="backpack-card">
    ...
    <p className="backpack-help">ЦИФРА — выбрать предмет · ENTER — экипировать или использовать · E / У — закрыть.</p>
    <div className="item-list">
      {potions > 0 && (() => { const number = ++index; return <button className={`potion-item ${selected === number - 1 ? 'keyboard-selected' : ''}`} disabled={health >= maxHealth} onClick={() => { setSelected(number - 1); onUsePotion() }}><kbd>{number}</kbd><span className="health-potion backpack-potion"><i /></span><div><strong>Зелёное зелье ×{potions}</strong><small>Восстанавливает 50 HP.</small></div><b>{health >= maxHealth ? 'HP ПОЛНОЕ' : 'ИСПОЛЬЗОВАТЬ'}</b></button> })()}
