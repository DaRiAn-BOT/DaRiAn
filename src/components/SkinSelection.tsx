...
export default function SkinSelection({ selectedSkin, onSelect, onConfirm }: Props) {
  return <div className="story-card skin-selection">
    ...
    <div className="skin-grid" aria-label="Выбор героя">
      {skinNames.map((name, index) => <button key={name} className={`skin-option skin-${index} ${selectedSkin === index ? 'selected' : ''}`} onClick={() => onSelect(index)} aria-label={name} title={name}><i /><small>{index + 1}</small></button>)}
    </div>
